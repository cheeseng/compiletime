...
val scalaTestWithoutMacroVersion = "3.0.0-before"
val scalaTestWithMacroVersion = "3.0.0-after"
...
// Using assert(===)
def shouldBeShorthandTestBodyFun(x: Int): String = x + " + 1 shouldBe " + (x+1)
...
val scalatestOldJar = new File("scalatest-app_" + scalaVersion + "-" + scalaTestWithoutMacroVersion + ".jar")
if (!scalatestOldJar.exists)
  downloadFile("https://oss.sonatype.org/content/repositories/releases/org/scalatest/scalatest_" + scalaVersion + "/" + scalaTestWithoutMacroVersion + "/scalatest-app_" + scalaVersion + "-" + scalaTestWithoutMacroVersion + ".jar", scalatestOldJar)

val scalatestMacroJar = new File("scalatest-app_" + scalaVersion + "-" + scalaTestWithMacroVersion + ".jar")
if (!scalatestMacroJar.exists)
  downloadFile("https://oss.sonatype.org/content/repositories/releases/org/scalatest/scalatest_" + scalaVersion + "/" + scalaTestWithMacroVersion + "/scalatest-app_" + scalaVersion + "-" + scalaTestWithMacroVersion + ".jar", scalatestMacroJar)

val baseDir = new File("target/" + scalaVersion + "/matcherBeforeAfter")
if (baseDir.exists)
  deleteDir(baseDir)
...
val testTypes = 
  Array(
    TestType("Before should be", "BeforeShouldBe", Array("org.scalatest._", "Matchers._"), Array.empty, shouldBeTestBodyFun, scalatestOldJar),
    TestType("After should be", "AfterShouldBe", Array("org.scalatest._", "Matchers._"), Array.empty, shouldBeTestBodyFun, scalatestMacroJar), 
    TestType("Before shouldBe", "BeforeShouldBeShorthand", Array("org.scalatest._", "Matchers._"), Array.empty, shouldBeShorthandTestBodyFun, scalatestOldJar),
    TestType("After shouldBe", "AfterShouldBeShorthand", Array("org.scalatest._", "Matchers._"), Array.empty, shouldBeShorthandTestBodyFun, scalatestMacroJar)
  )
  ...
  testTypes.foreach { testType =>
    ...
    try {
      testCounts.foreach { testCount =>
        println("Working on " + testType.name + " test count " + testCount + "...")
